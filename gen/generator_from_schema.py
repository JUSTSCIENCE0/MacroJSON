...
def parse_schema_object(root : dict, objects : dict):
    ...
    if title != 'nullptr':
        if identifier in objects:
            if objects[identifier]["fields"] == object_descr["fields"]:
                return identifier
            else:
                identifier = identifier + "Alt" + g_objects_counter
    else:
        for id_key, descr in objects.items():
            if descr["fields"] == object_descr["fields"]:
                return id_key

    object_descr["identifier"] = identifier
    objects[identifier] = object_descr
    return identifier

def generate_code(objects: dict) -> str:
...
def main():
    ...
    objects_dict = {}
    parse_schema_object(json_schema, objects_dict)
    print(objects_dict)

    code = generate_code(objects_dict)
    print(code)
